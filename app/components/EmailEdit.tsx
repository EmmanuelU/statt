...
async function updateEmails(emails: string[]) {
    const jsonObject = { emails };
...
export default function Chat() {
    ...
    const validateEmails = (emailsToValidate: string[]) => {
    ...
    const saveChanges = async () => {
        if (validateEmails(emails)) {
            ...
            alert("Please correct the invalid email addresses before saving.");
        }
    };
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (example === "Go Back to StattAI.") {
                                    if (!hasUnsavedChanges || window.confirm("You have unsaved emails, are you sure?"))
                                        router.push("/")
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                        {emails.map((email, index) => (
                            <div key={index} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full flex items-center">
                                ...
                                {errors[`email${index}`] && (
                                    <span className="text-red-500 text-xs ml-2">{errors[`email${index}`]}</span>
                                )}
                            </div>
                        ))}
                    </div>
